...
def compute_stats_plus(
    ...
    trades = (
        [t for t in chain(*trades.values())] if isinstance(trades, dict) else trades
    )
    if isinstance(trades, pd.DataFrame):
        trades_df: pd.DataFrame = trades
    else:
    ...
    # Assuming equal weights for each asset
    percent_returns = []
    for key, data in ohlc_data.items():
        c = data.Close.values
        percent_returns.append((c[-1] - c[0]) / c[0] * 100)

    s.loc["Buy & Hold Return [%]"] = np.mean(percent_returns)
