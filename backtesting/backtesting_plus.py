...
class _Orders(tuple):
    ...
    def __getattr__(self, item):
        ...
        if item in removed_attrs:
            raise AttributeError(f'StrategyPlus.orders.{"/.".join(removed_attrs)} were removed in'
                                 'Backtesting 0.2.0. '
...
class BacktestPlus:
    ...
    def __init__(self,
        ...
        if not isinstance(data, Dict):
            raise TypeError("`data` must be a dict of str:pandas.DataFrame with columns")
        else:
        ...
        self._broker = partial(
            ...
            exclusive_orders=exclusive_orders, index=index,
        )
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        """
        data = _Data(self._data.copy(deep=False)) #TODO: make a _DataPlus class that can handle multiple dataframes
        broker: _Broker = self._broker(data=data)
        strategy: StrategyPlus = self._strategy(broker, data, kwargs)

        strategy.init()
        data._update()  # StrategyPlus.init might have changed/added to data.df
