...
class StrategyPlus(metaclass=ABCMeta):
    """
    A trading strategy base class. Extend this class and
    override methods
    `backtesting.backtesting.StrategyPlus.init` and
    `backtesting.backtesting.StrategyPlus.next` to define
    your own strategy.
    """

    def __init__(self, broker, data, params, symbol: str):
        self._indicators = []
        ...
        for k, v in params.items():
            if not hasattr(self, k):
                raise AttributeError(
                    f"StrategyPlus '{self.__class__.__name__}' is missing parameter '{k}'."
                    "StrategyPlus class should define parameters as class variables before they "
    ...
    def I(
        ...
        **kwargs,
        ...
        `backtesting.backtesting.StrategyPlus.next` much like
        `backtesting.backtesting.StrategyPlus.data` is.
        Returns `np.ndarray` of indicator values.

        `func` is a function that returns the indicator array(s) of
        same length as `backtesting.backtesting.StrategyPlus.data`.
    ...
    def init(self):
        ...
        Declare indicators (with `backtesting.backtesting.StrategyPlus.I`).
    ...
    def next(self):
        ...
        `backtesting.backtesting.StrategyPlus.data`
        instance (row; full candlestick bar) becomes available.
        This is the main method where strategy decisions
        upon data precomputed in `backtesting.backtesting.StrategyPlus.init`
        take place.
    ...
    def buy(
        ...
        *,
        ...
        See also `StrategyPlus.sell()`.
        """
        assert (
            0 < size < 1 or round(size) == size
        ), "size must be a positive fraction of equity, or a positive whole number of units"
        return self._broker.new_order(self.symbol, size, limit, stop, sl, tp, tag)
    ...
    def position(self) -> "Position":
        ...
        return self._broker.positions[self.symbol]

    @property
    def orders(self) -> "Tuple[Order, ...]":
        """List of orders (see `Order`) waiting for execution."""
        return _Orders(self._broker.orders[self.symbol])

    @property
    def trades(self) -> "Tuple[Trade, ...]":
        """List of active trades (see `Trade`)."""
        return tuple(self._broker.trades[self.symbol])

    @property
    def closed_trades(self) -> "Tuple[Trade, ...]":
        """List of settled trades (see `Trade`)."""
        return tuple(self._broker.closed_trades[self.symbol])
...
class _Orders(tuple):
    ...
    def __getattr__(self, item):
        ...
        if item in removed_attrs:
            raise AttributeError(
                f'StrategyPlus.orders.{"/.".join(removed_attrs)} were removed in'
                "Backtesting 0.2.0. "
...
class Position:
    ...
    `backtesting.backtesting.StrategyPlus.position` within
    `backtesting.backtesting.StrategyPlus.next`.
    ...
    def __init__(self, broker: "_Broker", symbol: str):
    ...
    def size(self) -> float:
        ...
        return sum(trade.size for trade in self.__broker.trades[self.symbol])

    @property
    def pl(self) -> float:
        """Profit (positive) or loss (negative) of the current position in cash units."""
        return sum(trade.pl for trade in self.__broker.trades[self.symbol])

    @property
    def pl_pct(self) -> float:
        """Profit (positive) or loss (negative) of the current position in percent."""
        weights = np.abs([trade.size for trade in self.__broker.trades[self.symbol]])
        weights = weights / weights.sum()
        pl_pcts = np.array(
            [trade.pl_pct for trade in self.__broker.trades[self.symbol]]
        )
    ...
    def close(self, portion: float = 1.0):
        ...
        """
        for trade in self.__broker.trades[self.symbol]:
            trade.close(portion)

    def __repr__(self):
        return f"<Position: {self.symbol} {self.size} ({len(self.__broker.trades[self.symbol])} trades)>"
...
class Order:
    """
    Place new orders through `StrategyPlus.buy()` and `StrategyPlus.sell()`.
    Query existing orders through `StrategyPlus.orders`.
    ...
    def cancel(self):
        """Cancel the order."""
        self.__broker.orders[self.__symbol].remove(self)
...
class Trade:
    ...
    Find active trades in `StrategyPlus.trades` and closed, settled trades in `StrategyPlus.closed_trades`.
    """
    ...
    def close(self, portion: float = 1.0):
        ...
        order = Order(
            self.__broker, self.__symbol, size, parent_trade=self, tag=self.__tag
        )
        self.__broker.orders[self.__symbol].insert(0, order)
    ...
    def entry_time(self) -> Union[pd.Timestamp, int]:
        ...
        return self.__broker._data[self.__symbol].index[self.__entry_bar]
    ...
    def exit_time(self) -> Optional[Union[pd.Timestamp, int]]:
        ...
        if self.__exit_bar is None:
            return None
        return self.__broker._data[self.__symbol].index[self.__exit_bar]
    ...
    def __set_contingent(self, type, price):
        ...
        if price:
            ...
            order = self.__broker.new_order(
                self.symbol, -self.size, trade=self, tag=self.tag, **kwargs
            )
...
class _Broker:
    def __init__(
        ...
        *,
        ...
        self.positions: Dict[str, Position] = {
            symbol: Position(self, symbol=symbol) for symbol in self._data.keys()
        }
        self.closed_trades: Dict[str, List[Trade]] = defaultdict(list)

    def __repr__(self):
        pl = sum([pos.pl for pos in self.positions.values()])
        n_trades = sum([len(t) for t in self.trades.values()])
        return f"<Broker: {self._cash:.0f}{pl:+.1f} ({n_trades} trades)>"
    ...
    def new_order(
        ...
        *,
        ...
        is_long = size > 0
        adjusted_price = self._adjusted_price(symbol=symbol, size=size)
        ...
        order = Order(self, symbol, size, limit, stop, sl, tp, trade, tag)
        # Put the new order in the order queue,
        # inserting SL/TP/trade-closing orders in-front
        if trade:
            self.orders[symbol].insert(0, order)
        else:
    ...
    def last_price(self, symbol: str) -> float:
        ...
        return self._data[symbol].Close[-1]

    def _adjusted_price(self, symbol: str, size=None, price=None) -> float:
        """
        Long/short `price`, adjusted for commisions.
        In long positions, the adjusted price is a fraction higher, and vice versa.
        """
        return (price or self.last_price(symbol=symbol)) * (
            1 + copysign(self._commission, size)
        )

    @property
    def equity(self) -> float:
        pl_sum = sum(trade.pl for trade in chain(*self.trades.values()))
        return self._cash + pl_sum

    @property
    def margin_available(self) -> float:
        # From https://github.com/QuantConnect/Lean/pull/3768
        margin_used = sum(
            trade.value / self._leverage for trade in chain(*self.trades.values())
        )
        return max(0, self.equity - margin_used)

    def next(self, symbol: str):
        i = self._i = len(self._data[symbol]) - 1
        self._process_orders(symbol=symbol)
        ...
        if equity <= 0:
            ...
            for trade in chain(*self.trades.values()):
                self._close_trade(trade, self._data[trade.symbol].Close[-1], i)
            self._cash = 0
            self._equity[i:] = 0
    ...
    def _process_orders(self, symbol: str):
        ...
        # Process orders
        for order in list(self.orders[symbol]):  # type: Order

            # Related SL/TP order was already removed
            if order not in self.orders[symbol]:
                continue
            ...
            if order.parent_trade:
                ...
                if trade in self.trades[symbol]:
                    self._reduce_trade(trade, price, size, time_index)
                    assert order.size != -_prev_size or trade not in self.trades[symbol]
                if order in (trade._sl_order, trade._tp_order):
                    assert order.size == -trade.size
                    assert (
                        order not in self.orders[symbol]
                    )  # Removed when trade was closed
                else:
            ...
            # In long positions, the adjusted price is a fraction higher, and vice versa.
            adjusted_price = self._adjusted_price(symbol, order.size, price)
            ...
            if not self._hedging:
                ...
                for trade in list(self.trades[symbol]):
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int):
        ...
        if not size_left:
            ...
            self.trades[trade.symbol].append(close_trade)

        self._close_trade(close_trade, price, time_index)

    def _close_trade(self, trade: Trade, price: float, time_index: int):
        self.trades[trade.symbol].remove(trade)
        if trade._sl_order:
            self.orders[trade.symbol].remove(trade._sl_order)
        if trade._tp_order:
            self.orders[trade.symbol].remove(trade._tp_order)

        self.closed_trades[trade.symbol].append(
    ...
    def _open_trade(
        ...
        trade = Trade(self, symbol, size, price, time_index, tag)
...
class BacktestPlus:
    ...
    def __init__(
        ...
        *,
        ...
        `strategy` is a `backtesting.backtesting.StrategyPlus`
        ...
        if not (isinstance(strategy, type) and issubclass(strategy, StrategyPlus)):
            raise TypeError("`strategy` must be a StrategyPlus sub-type")
        ...
        if not isinstance(data, Dict):
            raise TypeError(
                "`data` must be a dict of str:pandas.DataFrame with columns"
            )
        else:
            for key, value in data.items():
                ...
                if not isinstance(value, pd.DataFrame):
                    raise TypeError("`data` values must be pandas.DataFrame")

        data = data.copy()
        data = self._check_data(data, cash=cash)

        self._data: Dict[str, pd.DataFrame] = data
        # get the index of the first data frame (they should all be the same)
    ...
    @staticmethod
    def _check_data(data_dict: Dict[str, pd.DataFrame], cash: float) -> pd.DataFrame:
        output = {}
        index_len = None
        index_min = None
        index_max = None
        for key, data in data_dict.items():
            ...
            if index_len is None:
                assert index_min is None
                assert index_max is None
                index_len = len(data)
                index_min = data.index.min()
                index_max = data.index.max()
            else:
                if len(data) != index_len:
                    raise ValueError("All dataframes must have the same length")
                if data.index.min() != index_min:
                    raise ValueError("All dataframes must have the same starting index")
    ...
    def _get_start_from_indicator_attr(
        self, indicator_attr: Dict[str, _Indicator]
    ) -> int:
        """
        Calculates the start index for a set of indicators after warmup period.
        Essentially the start index is the maximum of the warmup periods of all indicators.
        """
        start = 1 + max(
            (
                np.isnan(indicator.astype(float)).argmin(axis=-1).max()
                for _, indicator in indicator_attr.items()
            ),
            default=0,
        )
        return start
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        .. warning::
            ...
            period of the `StrategyPlus.I` indicator which lags the most.
            Obviously, this can affect results.
        """
        data = _DataPlus(self._data.copy())
        broker: _Broker = self._broker(data=data)
        strategies: Dict[str, StrategyPlus] = {
            symbol: self._strategy(broker, d, kwargs, symbol=symbol)
            for symbol, d in data.items()
        }

        [s.init() for s in strategies.values()]
        [
            d._update() for d in data.values()
        ]  # StrategyPlus.init might have changed/added to data.df

        # Indicators used in StrategyPlus.next()
        indicator_attrs = {
            symbol: {
                attr: indicator
                for attr, indicator in s.__dict__.items()
                if isinstance(indicator, _Indicator)
            }
            for symbol, s in strategies.items()
        }
        # Skil first few candles where indicators are still "warming up" for each asset
        # +1 to have at least two entries available
        starts = {
            symbol: self._get_start_from_indicator_attr(indicator_attr)
            for symbol, indicator_attr in indicator_attrs.items()
        }
        # Disable "invalid value encountered in ..." warnings. Comparison
        # np.nan >= 3 is not invalid; it's False.
        with np.errstate(invalid="ignore"):
            dones = {symbol: False for symbol in data.keys()}
            out_of_money = False
            while not out_of_money and not all(dones.values()):
                for symbol, start in starts.items():
                    try:
                        data[symbol]._set_length(start + 1)
                    except ValueError:
                        dones[symbol] = True
                        continue
                    for attr, indicator in indicator_attrs[symbol].items():
                        # Slice indicator on the last dimension (case of 2d indicator)
                        setattr(strategies[symbol], attr, indicator[..., : start + 1])

                    # Handle orders processing and broker stuff
                    try:
                        broker.next(symbol=symbol)
                    except _OutOfMoneyError:
                        out_of_money = True
                        break
                    # Next tick, a moment before bar close
                    strategies[symbol].next()
                    # increment the start index
                    starts[symbol] = start + 1

            if not out_of_money:
                # Close any remaining open trades so they produce some stats
                for trade in chain(*broker.trades.values()):
                    trade.close()
                ...
                for symbol, start in starts.items():
                    try_(
                        partial(broker.next, symbol=symbol),
                        exception=_OutOfMoneyError,
                    )
