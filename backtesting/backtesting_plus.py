...
class _Broker:
    ...
    def __repr__(self):
        pl = sum([self.positions[symbol].pl for symbol in self._data.keys()])
        n_trades = sum([len(self.trades[symbol]) for symbol in self._data.keys()])
