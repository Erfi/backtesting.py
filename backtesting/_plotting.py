...
def _maybe_resample_data_plus(
    resample_rule: Union[bool, str],
    df: Dict[str, pd.DataFrame],
    indicators: Dict[str, List[_Indicator]],
    ...
    if isinstance(resample_rule, str):
        ...
        if resample_rule is False or np.any(
            np.array([len(d) for d in df.values()]) <= _MAX_CANDLES
        ):
            logging.warning(
                f"At least one dataframe has less than {_MAX_CANDLES} candles. Not resampling."
            )
            return df, indicators, equity_data, trades
        ...
        # assuming all dataframes have similar length, we can resample them all the same
        sample_df = next(iter(df.values()))
        timespan = sample_df.index[-1] - sample_df.index[0]
        require_minutes = (timespan / _MAX_CANDLES).total_seconds() // 60
        freq = freq_minutes.where(freq_minutes >= require_minutes).first_valid_index()
        warnings.warn(
            f"Data contains too many candlesticks to plot; downsampling to {freq!r}. "
...
def plot_plus(
    *,
    ...
    # TestPlot.test_file_size() test was failing).
    if not filename and not IS_JUPYTER_NOTEBOOK:
        filename = _windos_safe_filename("portfolio_plot")
    _bokeh_reset(filename)
    ...
    sample_df = next(iter(df.values()))  # assimung all dataframes have similar content
    plot_volume = plot_volume and not sample_df.Volume.isnull().all()
    plot_equity = plot_equity and not trades.empty
    plot_return = plot_return and not trades.empty
    plot_pl = plot_pl and not trades.empty
    is_datetime_index = isinstance(sample_df.index, pd.DatetimeIndex)

    from .lib import OHLCV_AGG

    # ohlc df may contain many columns. We're only interested in, and pass on to Bokeh, these
    df = {key: d[list(OHLCV_AGG.keys())].copy(deep=False) for key, d in df.items()}

    # Limit data to max_candles
    if is_datetime_index:
        df, indicators, equity_data, trades = _maybe_resample_data_plus(
            resample, df, indicators, equity_data, trades
        )

    for key, d in df.items():
        d.index.name = None  # Provides source name @index
        d["datetime"] = d.index  # Save original, maybe datetime index
        df[key] = d.reset_index(drop=True)
    equity_data = equity_data.reset_index(drop=True)
    sample_df = next(iter(df.values()))
    index = sample_df.index

    all_plots = []
    equity_plot_present = False
    return_plot_present = False
    drawdown_plot_present = False
    # make a new figure for every asset
    for key, d in df.items():
        new_bokeh_figure = partial(
            _figure,
            x_axis_type="linear",
        ...
        source.add((d.Close >= d.Open).values.astype(np.uint8).astype(str), "inc")
        trade_source = ColumnDataSource(
            dict(
    ...
    return this.labels[index] || "";
        ...
        NBSP = "\N{NBSP}" * 4  # noqa: E999
        ohlc_extreme_values = d[["High", "Low"]].copy(deep=False)
        ohlc_tooltips = [
            ("x, y", NBSP.join(("$index", "$y{0,0.0[0000]}"))),
            (
                "OHLC",
        ...
        def _plot_equity_section(is_return=False):
            ...
            if smooth_equity:
                ...
                select = pd.Index(trades[trades.Symbol == key]["ExitBar"]).union(
                    interest_points
                )
                select = select.unique().dropna()
                equity = equity.iloc[select].reindex(equity.index)
            ...
            # Equity line
            r = fig.line(
                "index", source_key, source=source, line_width=1.5, line_alpha=1
            )
            if relative_equity:
                tooltip_format = f"@{source_key}{{+0,0.[000]%}}"
                tick_format = "0,0.[00]%"
            ...
            dd_timedelta_label = (
                d["datetime"].iloc[int(round(dd_end))] - d["datetime"].iloc[dd_start]
            )
            fig.line(
                [dd_start, dd_end],
                equity.iloc[dd_start],
        ...
        def _plot_pl_section():
            ...
            returns_long = np.where(
                trades[trades.Symbol == key]["Size"] > 0,
                trades[trades.Symbol == key]["ReturnPct"],
                np.nan,
            )
            returns_short = np.where(
                trades[trades.Symbol == key]["Size"] < 0,
                trades[trades.Symbol == key]["ReturnPct"],
                np.nan,
            )
            size = trades[trades.Symbol == key]["Size"].abs()
            size = np.interp(size, (size.min(), size.max()), (8, 20))
            trade_source.add(returns_long, "returns_long")
            trade_source.add(returns_short, "returns_short")
            trade_source.add(size, "marker_size")
            if "count" in trades[trades.Symbol == key]:
                trade_source.add(trades[trades.Symbol == key]["count"], "count")
            r1 = fig.scatter(
                "index",
                "returns_long",
                source=trade_source,
        ...
        def _plot_superimposed_ohlc():
            """Superimposed, downsampled vbars"""
            time_resolution = pd.DatetimeIndex(d["datetime"]).resolution
            resample_rule = (
                superimpose
                if isinstance(superimpose, str)
                else dict(
                    day="M", hour="D", minute="H", second="T", millisecond="S"
                ).get(time_resolution)
            )
            if not resample_rule:
                warnings.warn(
            ...
            # Check if resampling was downsampling; error on upsampling
            orig_freq = _data_period(d["datetime"])
            resample_freq = _data_period(df2.index)
            if resample_freq < orig_freq:
                raise ValueError(
                    "Invalid value for `superimpose`: Upsampling not supported."
        ...
        def _plot_ohlc_trades():
            """Trade entry / exit markers on OHLC plot"""
            trade_source.add(
                trades[trades.Symbol == key][["EntryBar", "ExitBar"]].values.tolist(),
                "position_lines_xs",
            )
            trade_source.add(
                trades[trades.Symbol == key][
            ...
            fig_ohlc.multi_line(
                ...
                source=trade_source,
                line_color=trades_cmap,
                legend_label=f"Trades ({len(trades[trades.Symbol == key])})",
                line_width=8,
                line_alpha=1,
                line_dash="dotted",
            )
        ...
        def _plot_indicators():
            ...
            for i, value in enumerate(indicators[key]):
                value = np.atleast_2d(value)
        ...
        # Construct figure ...
        if plot_equity and not equity_plot_present:
            _plot_equity_section()
            equity_plot_present = True

        if plot_return and not return_plot_present:
            _plot_equity_section(is_return=True)
            return_plot_present = True

        if plot_drawdown and not drawdown_plot_present:
            figs_above_ohlc.append(_plot_drawdown_section())
            drawdown_plot_present = True
